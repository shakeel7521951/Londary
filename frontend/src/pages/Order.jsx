...
const OrderPage = () => {
  ...
  const garmentTypes = {
    "Men's": ['Kandura', 'Shirt', 'Trousers', 'Bisht', 'Thobe', 'Socks', 'Underwear'],
    "Women's": ['Abaya', 'Dress', 'Blouse', 'Skirt', 'Scarf', 'Hijab', 'Lingerie']
  };
  ...
  const packagingOptions = [
    { id: 'plastic', name: 'Plastic Wrap', desc: 'Clean transparent protection' },
    { id: 'fabric', name: 'Luxury Fabric Wrap', desc: 'Soft-touch premium wrapping' },
    { id: 'box', name: 'Premium Gift Box', desc: 'Elegant box with magnetic closure' }
  ];
  ...
  const Step1 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">How would you like us to clean your garments?</h3>
        ...
        {washOptions.map((option) => (
          <motion.div
            ...
            className={`p-6 border rounded-xl cursor-pointer transition-all ${
              washType === option.id ? 'border-[#D4AF37] bg-[#FFF9E6]' : 'border-gray-200 hover:border-gray-300'
            }`}
            onClick={() => setWashType(option.id)}
          >
            <div className="flex justify-between items-center">
              <h4 className="font-medium">{option.name}</h4>
              <span className="text-[#D4AF37] font-bold">{option.price} QAR</span>
            </div>
          </motion.div>
        ))}
      </div>
  ...
  const Step2 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">What clothes are you sending us?</h3>
          ...
          <div key={category}>
            ...
            <div className="space-y-2">
              {items.map((item) => (
                ...
                >
                  <span>{item}</span>
                  <svg className="w-5 h-5 ml-auto text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const Step3 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">Do you want us to steam the garments?</h3>
  ...
  const Step4 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">Would you like us to perfume your clothes with a luxury scent?</h3>
      <div className="grid gap-4 md:grid-cols-2">
              ...
              {options.map((option) => (
                <motion.button
                  ...
                  className={`w-full p-4 text-left border rounded-lg ${
                    fragrance === option.id ? 'border-[#D4AF37] bg-[#FFF9E6]' : 'border-gray-200 hover:border-gray-300'
                  }`}
  ...
  const Step5 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">How would you like us to package your garments?</h3>
        ...
        {packagingOptions.map((option) => (
          <motion.div
            ...
            className={`p-6 border rounded-xl cursor-pointer ${
              packaging === option.id ? 'border-[#D4AF37] bg-[#FFF9E6]' : 'border-gray-200 hover:border-gray-300'
            }`}
            onClick={() => setPackaging(option.id)}
          >
            <h4 className="font-medium">{option.name}</h4>
            <p className="text-sm text-gray-600 mt-1">{option.desc}</p>
            {option.id === 'fabric' && <p className="text-[#D4AF37] text-sm mt-2">+15 QAR</p>}
            {option.id === 'box' && <p className="text-[#D4AF37] text-sm mt-2">+30 QAR</p>}
          </motion.div>
        ))}
      </div>
    </div>
  ...
  const Step6 = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-light text-gray-700">Want to include a personalized card?</h3>
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            onChange={(e) => setCardDetails({...cardDetails, from: e.target.value})}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setCardDetails({...cardDetails, to: e.target.value})}
            className="w-full p-3 border border-gray-300 rounded-lg"
          />
        </div>
      </div>
  ...
  const Step7 = () => (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        <div className="space-y-4">
          <div className="flex justify-between border-b pb-2">
            ...
            <span>{washType === 'standard' ? 'Standard Wash' : 'Express Wash'}</span>
          </div>
          ...
          {fragrance && (
            <div className="flex justify-between border-b pb-2">
              <span>Fragrance:</span>
              <span>{fragranceOptions[fragrance.includes('orchid') ? "Women's" : "Men's"]
                .find(f => f.id === fragrance).name}</span>
            </div>
          )}
          ...
          {packaging && (
            <div className="flex justify-between border-b pb-2">
              <span>Packaging:</span>
              <span>{packagingOptions.find(p => p.id === packaging).name} 
                {packaging === 'fabric' ? ' (+15 QAR)' : packaging === 'box' ? ' (+30 QAR)' : ''}</span>
            </div>
          )}
      ...
      <motion.button
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        className="w-full py-3 bg-[#D4AF37] text-white rounded-lg font-bold"
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f9f7f4] to-[#f1ece5] py-12 pt-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-6 bg-[#1C1C1C] text-center">
          <h2 className="text-2xl font-light text-[#D4AF37]">AKOYA PREMIUM LAUNDRY</h2>
            ...
            {step < 7 && (
              <motion.button
                ...
                className={`ml-auto px-6 py-2 rounded-lg ${
                  ...
                  'bg-gray-300 cursor-not-allowed' : 'bg-[#D4AF37] text-white'
                }`}
                onClick={() => setStep(step + 1)}
                disabled={
                  (step === 1 && !washType) || 
